...
public class CallMonoColorBarChartMinigameTime : MonoBehaviour
{
    [SerializeField]
    ...
    public void GenerateMonoColorBarChartValues()
    {
        ...
        {
            double days = 0;
            days = (DateTime.Now - userMinigameStats[i].EndTime).Days;
            if (days < 10)
            {
                charts[(long)(10 - 1 - days)] += (float)(userMinigameStats[i].EndTime - userMinigameStats[i].BeginTime).TotalSeconds;
            }
        }
