...
public class CallMonoColorBarChartMaterialTime : MonoBehaviour
{
    [SerializeField]
    private PiePart piePart;

    private List<UserOpennedMaterial> userOpennedMaterials;
    ...
    public void GetData(List<UserOpennedMaterial> userOpennedMaterials)
    {
    ...
    public void GenerateMonoColorBarChartValues()
    {
        ...
        for (int i = 0; i < userOpennedMaterials.Count; i++)
        {
            double days = 0;
            days = (DateTime.Now - userOpennedMaterials[i].OpenningDate).Days;
            if (days < 10)
            {
